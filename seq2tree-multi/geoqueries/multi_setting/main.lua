...
function setup_multi()
  ...
  local word_manager_vocab_size_list = {}
  for n = 1, num_enc do
    table.insert(word_manager_vocab_size_list, symbol_managers[n].vocab_size)
  end
...
function main()
  ...
  cmd:option('-dropoutrec',0.1,'dropout for regularization, used after each c_i. 0 = no dropout')
  ...
  cmd:option('-max_epochs',90,'number of full passes through the training data')
  -- optimization
  ...
  cmd:option('-restart',20,'in number of epochs, when to restart the optimization')
  ...
  for i = 1, iterations do
    ...
    if opt.opt_method == 0 then
      _, loss = optim.rmsprop(eval_training_multi, param_x, optim_state)
    elseif opt.opt_method == 1 then
      _, loss = optim.sgd(eval_training_multi, param_x, optim_state)
    end
    ...
    -- on last iteration
    if i == iterations or epoch % opt.save_every == 0 then
      local checkpoint = {}
      checkpoint.enc_rnn_unit = model.enc_rnn_unit
      checkpoint.dec_rnn_unit = model.dec_rnn_unit
      checkpoint.dec_att_unit = model.dec_att_unit
      ...
      if i == iterations then
        ...
        torch.save(string.format('%s/model-%d.t7', opt.checkpoint_dir, i), checkpoint)
      end
    end
