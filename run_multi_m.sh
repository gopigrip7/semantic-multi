set -e -x
# ./run_multi_m.sh seq2tree-multi [geoqueries|atis] multi_setting lang1,lang2,...,langN param_file [word|sent] param_id
...
if [ $3 = "single_setting" ] || [ $3 = "multi_setting" ]; then
  ...
  while read -r param; do
    ...
    if [ $prefix -eq $7 ] ; then
      ...
      $param > $logfile 2>&1
      CUDA_VISIBLE_DEVICES=$GPU_ID th $3/sample.lua -data_dir $WORK_DIR/data/$exp_name -model $DUMP_DIR/model.t7 -sample 0 -display 0 >> $logfile 2>&1
    fi
  done < $PWD_DIR/$5
  cd $PWD_DIR
fi
